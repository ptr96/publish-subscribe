...
def publisher_window():

    print("Publisher window opened")

    with dpg.window(tag="Publisher Window", label="Publisher window publish-subscribe app", autosize=True):

        dpg.set_viewport_height(700)
        dpg.set_viewport_width(700)
        dpg.set_primary_window("Login window", True)
        dpg.add_text("Hello, User!")
        dpg.add_text("Here You can add news to the topic you are interested in:\n\n")
        dpg.add_text("Topics:\n")
...
def subscriber_window():

    print("Subscriber window opened")

    with dpg.window(tag="Subscriber Window", label="Subscriber Window publish-subscribe app", autosize=True):
...
with dpg.window(tag="Login window",label="Client publish-subscribe app",autosize=True):
    dpg.set_primary_window("Login window", True)
    dpg.set_viewport_height(200)
    dpg.set_viewport_width(500)
    dpg.add_text("Hello, User!")
    dpg.add_text("Choose which function you want to perform on the server:\n\n")
    dpg.add_button(label="Publisher", callback=publisher_window)
    dpg.add_button(label="Subscriber", callback=subscriber_window)
